<script setup>
import { Link } from '@inertiajs/vue3';
import { useCurrentTime } from '@/Composables/UseCurrentTime.js';

const { currentTime } = useCurrentTime();
const props = defineProps({
    UpcomingReservations: Object,
    RoomCount: Object
});
</script>
<template>
<!-- Contenedor Principal -->
<div class="lg:flex gap-4 items-stretch">
    <!-- Caja Grande -->
    <div class="bg-white p-6 rounded-lg border border-gray-200 mb-4 lg:mb-0 shadow-md lg:w-[35%]">
        <div class="flex justify-center items-center space-x-5 h-full">
            <div>
                <p>Proximas llegadas</p>
                <h2 class="text-4xl font-bold text-gray-600">{{ props.UpcomingReservations.total ?? 'No disponibles' }}</h2>
                <p>{{ currentTime }}</p>
            </div>
            <!-- <img src="https://www.emprenderconactitud.com/img/Wallet.png" alt="wallet" class="h-24 md:h-20 w-38"> -->
            <div class="pl-4">
                <svg width="80px" height="80px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M8.13602 1.6026C8.35556 1.95386 8.24877 2.41657 7.89752 2.6361L3.8975 5.1361C3.54624 5.35563 3.08353 5.24885 2.864 4.8976C2.64447 4.54634 2.75125 4.08363 3.1025 3.8641L7.10253 1.3641C7.45378 1.14457 7.91649 1.25135 8.13602 1.6026ZM15.864 1.6026C16.0835 1.25135 16.5462 1.14457 16.8975 1.3641L20.8975 3.8641C21.2488 4.08363 21.3555 4.54635 21.136 4.8976C20.9165 5.24885 20.4538 5.35563 20.1025 5.1361L16.1025 2.6361C15.7512 2.41657 15.6445 1.95385 15.864 1.6026ZM12 4.7501C7.44365 4.7501 3.75 8.44375 3.75 13.0001C3.75 17.5564 7.44365 21.2501 12 21.2501C16.5563 21.2501 20.25 17.5564 20.25 13.0001C20.25 8.44375 16.5563 4.7501 12 4.7501ZM2.25 13.0001C2.25 7.61532 6.61522 3.2501 12 3.2501C17.3848 3.2501 21.75 7.61532 21.75 13.0001C21.75 18.3849 17.3848 22.7501 12 22.7501C6.61522 22.7501 2.25 18.3849 2.25 13.0001ZM12 8.2501C12.4142 8.2501 12.75 8.58589 12.75 9.0001V12.6894L15.0303 14.9698C15.3232 15.2627 15.3232 15.7375 15.0303 16.0304C14.7374 16.3233 14.2626 16.3233 13.9697 16.0304L11.4697 13.5304C11.329 13.3898 11.25 13.199 11.25 13.0001V9.0001C11.25 8.58589 11.5858 8.2501 12 8.2501Z" fill="#5a5a64"></path> </g></svg>
            </div>
        </div>
    </div>

    <!-- Caja Blanca -->
    <div class="bg-white font-bold p-4 rounded-lg xs:mb-4 max-w-full shadow-md lg:w-[65%]"> 
        <!-- Cajas pequeñas -->
        <div class="flex flex-wrap justify-between h-full">
            <!-- Caja pequeña 1 -->
            <div class="hover:cursor-pointer hover:shadow-select transition-all duration-300 flex-1 bg-gradient-to-r from-indigo-400 to-indigo-600 rounded-lg flex flex-col items-center justify-center p-4 space-y-2 border border-gray-200 m-2">
                <Link :href="route('admin.reservations.index')" class="w-full h-full flex flex-col justify-center items-center">
                    <svg width="48px" height="48px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M5.25013 6C5.25013 3.37665 7.37678 1.25 10.0001 1.25C12.6235 1.25 14.7501 3.37665 14.7501 6C14.7501 8.62335 12.6235 10.75 10.0001 10.75C7.37678 10.75 5.25013 8.62335 5.25013 6ZM10.0001 2.75C8.20521 2.75 6.75013 4.20507 6.75013 6C6.75013 7.79493 8.20521 9.25 10.0001 9.25C11.7951 9.25 13.2501 7.79493 13.2501 6C13.2501 4.20507 11.7951 2.75 10.0001 2.75Z" fill="#ffffff"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M3.97558 13.6643C5.55506 12.7759 7.68658 12.25 10.0001 12.25C12.3137 12.25 14.4452 12.7759 16.0247 13.6643C17.5807 14.5396 18.7501 15.8661 18.7501 17.5L18.7502 17.602C18.7513 18.7638 18.7527 20.222 17.4737 21.2635C16.8443 21.7761 15.9637 22.1406 14.7739 22.3815C13.5809 22.6229 12.0259 22.75 10.0001 22.75C7.97436 22.75 6.4194 22.6229 5.22634 22.3815C4.03661 22.1406 3.15602 21.7761 2.52655 21.2635C1.24752 20.222 1.24894 18.7638 1.25007 17.602L1.25013 17.5C1.25013 15.8661 2.41962 14.5396 3.97558 13.6643ZM4.71098 14.9717C3.37151 15.7251 2.75013 16.6487 2.75013 17.5C2.75013 18.8078 2.79045 19.544 3.47372 20.1004C3.84425 20.4022 4.46366 20.6967 5.52393 20.9113C6.58087 21.1252 8.02591 21.25 10.0001 21.25C11.9744 21.25 13.4194 21.1252 14.4763 20.9113C15.5366 20.6967 16.156 20.4022 16.5265 20.1004C17.2098 19.544 17.2501 18.8078 17.2501 17.5C17.2501 16.6487 16.6288 15.7251 15.2893 14.9717C13.9733 14.2315 12.1049 13.75 10.0001 13.75C7.89541 13.75 6.02693 14.2315 4.71098 14.9717Z" fill="#ffffff"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M16.6904 7.44694C17.411 7.12802 18.248 7.18847 19.0001 7.67889C19.7523 7.18847 20.5892 7.12802 21.3099 7.44694C22.1649 7.82533 22.7501 8.69929 22.7501 9.69973C22.7501 10.6481 22.3359 11.362 21.8396 11.9031C21.4522 12.3254 20.973 12.682 20.5946 12.9636C20.5133 13.0241 20.4367 13.0812 20.3666 13.1347L20.3646 13.1362C20.2159 13.2497 20.0236 13.3965 19.8209 13.5111C19.6169 13.6265 19.3345 13.75 19.0001 13.75C18.6658 13.75 18.3834 13.6265 18.1794 13.5111C17.9767 13.3965 17.7843 13.2497 17.6356 13.1362L17.6336 13.1347C17.5636 13.0812 17.487 13.0242 17.4058 12.9638C17.0274 12.6821 16.5481 12.3254 16.1607 11.9031C15.6644 11.362 15.2501 10.6481 15.2501 9.69973C15.2501 8.69929 15.8353 7.82533 16.6904 7.44694ZM16.7501 9.69973C16.7501 9.28775 16.99 8.95469 17.2974 8.81862C17.5637 8.7008 17.9875 8.68874 18.4683 9.17232C18.609 9.31392 18.8005 9.39354 19.0001 9.39354C19.1998 9.39354 19.3912 9.31392 19.532 9.17232C20.0128 8.68874 20.4366 8.7008 20.7029 8.81862C21.0103 8.95469 21.2501 9.28775 21.2501 9.69973C21.2501 10.1638 21.0614 10.5324 20.7342 10.8891C20.4522 11.1966 20.1158 11.448 19.7439 11.7259C19.6503 11.7959 19.5544 11.8676 19.4566 11.9423C19.2941 12.0663 19.1815 12.1495 19.0823 12.2056C19.0422 12.2283 19.0155 12.2399 19.0001 12.2456C18.9848 12.2399 18.9581 12.2283 18.918 12.2056C18.8188 12.1495 18.7062 12.0663 18.5437 11.9423C18.4459 11.8676 18.35 11.796 18.2564 11.726C17.8845 11.448 17.5481 11.1966 17.2661 10.8891C16.9388 10.5324 16.7501 10.1638 16.7501 9.69973Z" fill="#ffffff"></path> </g></svg>
                    <p class="text-white mt-2">Reservas</p>
                </Link>
            </div>

            <!-- Caja pequeña 2 -->
            <div class="hover:cursor-pointer hover:shadow-select transition-all duration-300 flex-1 bg-gradient-to-r from-sky-400 to-sky-600 rounded-lg flex flex-col items-center justify-center p-4 space-y-2 border border-gray-200 m-2">
                <Link :href="route('admin.rooms.rooms.index')" class="w-full h-full flex flex-col justify-center items-center">
                    <div>
                        <p class="text-white">Habitaciones hoy</p>
                        <p class="text-white">Ocupadas: {{ props.RoomCount.occupied }}</p>
                        <p class="text-white">Habitaciones: {{ props.RoomCount.available }}</p>
                    </div>
                </Link>
            </div>

            <!-- Caja pequeña 3 -->
            <div class="hover:cursor-pointer hover:shadow-select transition-all duration-300 flex-1 bg-gradient-to-r from-teal-400 to-teal-600 rounded-lg flex flex-col items-center justify-center p-4 space-y-2 border border-gray-200 m-2">
                <Link :href="route('admin.rooms.room-types.index')" class="w-full h-full flex flex-col justify-center items-center">
                    <svg width="48px" height="48px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M2.75 6C2.75 5.58579 2.41421 5.25 2 5.25C1.58579 5.25 1.25 5.58579 1.25 6V18C1.25 18.4142 1.58579 18.75 2 18.75C2.41421 18.75 2.75 18.4142 2.75 18V16.75L12 16.75L21.25 16.75V18C21.25 18.4142 21.5858 18.75 22 18.75C22.4142 18.75 22.75 18.4142 22.75 18V16.0048L22.75 16L22.75 15.9952L22.75 15.4694C22.7503 13.6408 22.7505 12.5245 22.4273 11.6009C21.8494 9.94923 20.5508 8.65062 18.8991 8.07267C17.9755 7.74949 16.8592 7.74967 15.0306 7.74998L14.7315 7.74993C14.0576 7.74936 13.5358 7.74892 13.0917 7.90432C12.3018 8.18073 11.6807 8.8018 11.4043 9.59173C11.2489 10.0358 11.2493 10.5577 11.2499 11.2315L11.25 15.25H2.75V6ZM21.2498 15.25H12.75V11.3571C12.75 10.4964 12.7606 10.2573 12.8201 10.0872C12.9458 9.72809 13.2281 9.44579 13.5871 9.32015C13.7573 9.26061 13.9964 9.25 14.8571 9.25C16.9148 9.25 17.7524 9.26061 18.4037 9.4885C19.6245 9.91567 20.5843 10.8755 21.0115 12.0963C21.2243 12.7045 21.2476 13.4751 21.2498 15.25Z" fill="#ffffff"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M7 7.75C5.20507 7.75 3.75 9.20507 3.75 11C3.75 12.7949 5.20507 14.25 7 14.25C8.79493 14.25 10.25 12.7949 10.25 11C10.25 9.20507 8.79493 7.75 7 7.75ZM5.25 11C5.25 10.0335 6.0335 9.25 7 9.25C7.9665 9.25 8.75 10.0335 8.75 11C8.75 11.9665 7.9665 12.75 7 12.75C6.0335 12.75 5.25 11.9665 5.25 11Z" fill="#ffffff"></path> </g></svg>
                    <p class="text-white">Habitaciones</p>
                </Link>
            </div>
        </div>
    </div>
</div>
</template>